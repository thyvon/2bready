...
function syncTokenCookie(token: string | null, fullyAuthenticated: boolean) {
  ...
  if (token) {
    document.cookie = `tp_auth_token=${token}; path=/; SameSite=Lax`;
  } else {
    document.cookie = 'tp_auth_token=; path=/; max-age=0';
  }
  // Separate from tp_auth_token: a pending-2FA session already holds a token
  ...
  if (fullyAuthenticated) {
    document.cookie = 'tp_auth_full=1; path=/; SameSite=Lax';
  } else {
    document.cookie = 'tp_auth_full=; path=/; max-age=0';
  }
}
