...
// in REDIRECT_IF_AUTHENTICATED_PATHS: the tp_auth_token cookie is already set
...
export function proxy(request: NextRequest) {
  ...
  // tp_auth_full is set only once 2FA (or a no-2FA login) fully completes —
  // see syncTokenCookie in auth.store.ts. tp_auth_token alone is not enough
  ...
  // there), looping.
  const isFullyAuthenticated = request.cookies.get('tp_auth_full')?.value === '1';
