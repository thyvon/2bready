...
export async function login(data: LoginInput): Promise<LoginResponse> {
  // tp-login, not the generic login every other app shares — the backend
  // rejects any account without portal.tp.access here before issuing a
  // token, rather than relying on this app's own post-login checks alone.
  const res = await api.post<{ data: LoginResponse }>('/auth/tp-login', data);
