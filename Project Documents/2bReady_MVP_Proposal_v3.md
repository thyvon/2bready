...
- ~~Taxonomy finalization (§7, item 3) is the single largest schedule risk in this plan, currently unresolved.~~ **RESOLVED (2026-07-11):** taxonomy is signed off and seeded (§7, item 3). This risk is closed — remaining taxonomy work is additive only (new country/industry seed data), not a blocking unknown.
...
These were flagged as Phase 0 exit criteria (§2.2). All four are now resolved (July 2026).
...
3. ~~Should the compliance framework/document taxonomy for P1–P4 be finalized and signed off before Sprint 4?~~ **RESOLVED (2026-07-11): Yes — the taxonomy is finalized.** The full P1–P4 document list (4 levels, 11 milestones, 38 documents, matching the reference concept's own taxonomy) is signed off as-is and now seeded as real `journey_templates`/`journey_levels`/`milestones`/`document_templates` rows (Sprint 4/5 work, built ahead of schedule against this exact list). **Standing rule going forward, per explicit owner direction: this taxonomy is never to be hardcoded outside the database.** It lives in one place — the seeded backend tables — and every consumer (admin-portal, client-portal) must read it from the real API. `journey_templates`/`document_templates` stay config-driven per country+industry (§1.8) so a second country/vertical's taxonomy is just new seed data, not new code.
