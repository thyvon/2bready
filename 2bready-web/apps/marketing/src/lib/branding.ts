...
async function fetchBranding(): Promise<BrandingSlots> {
  try {
    const res = await publicApi.get<{ data: BrandingSlots }>('/branding');
...
// Platform logos (business branding, admin-managed) for the header/footer.
// Uses the PUBLIC axios client — this page is anonymous and the
// authenticated one would redirect to /login on a 401.
...
const CACHE_KEY = 'marketing_branding_cache';
      ...
      void Promise.resolve()
        ...
        .then((cached) => {
          ...
          if (cached) {
            ...
            return force || !hasAny || Date.now() - cached.fetchedAt >= CACHE_TTL_MS ? fetchBranding() : null;
          }
...
/**
 * Logo for a theme-aware surface (header, footer): resolves the EFFECTIVE
