...
// in REDIRECT_IF_AUTHENTICATED_PATHS: the admin_auth_token cookie is already set
// the moment login issues a pending-2FA token (see auth.store.ts setPendingTotp),
...
export function proxy(request: NextRequest) {
  ...
  // admin_auth_full is set only once 2FA (or a no-2FA login) fully completes —
  // see syncTokenCookie in auth.store.ts. admin_auth_token alone is not enough
  // here: a pending-2FA session already holds a token cookie, and treating
  // that as "authenticated" would bounce it away from /login into a
  // /dashboard it can't actually render (isAuthenticated is still false
  // there), looping.
  const isFullyAuthenticated = request.cookies.get('admin_auth_full')?.value === '1';
