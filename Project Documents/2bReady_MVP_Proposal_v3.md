...
- **Journey templates are data, not code.** `journey_templates` (new table, parent of the existing `journeys`/`journey_levels`/`milestones`) lets a `country_code` + `industry_code` pair select a different P1–P4 taxonomy. Cambodia F&B ships as the first template at MVP; a second vertical (e.g. Cambodia manufacturing, or a second country) is a data-entry and translation exercise against the existing schema, not a schema change.
...
| 2 | Company Management | Company CRUD, bilingual profile (EN/KH), employee count + auto-bypass rule, status, progress view. **`industry_code`/`country_code` built as generic, non-Cambodia-locked fields per confirmed multi-vertical roadmap (§7, item 4).** | ★★★★ | Low |
...
| Entity | Purpose |
|---|---|
| `journey_templates` | Parent of `journeys`; keyed by `(country_code, industry_code)`. Enables a second vertical/country without a schema change (§1.8). |
...
4. ~~Is a second country or vertical planned within 12 months...~~ **RESOLVED (July 2026): Yes.** `journey_templates` (§1.8) is confirmed as a required MVP investment, not a nice-to-have — Company-model fields (`industry_code`, `country_code`) must stay generic from Sprint 2 onward and never assume Cambodia-F&B as a hardcoded default.
