...
- **Billing pricing must never depend on journey being present** — `buildLevelPricing` in `billing-data.ts` falls back to a synthetic `JourneyLevel` built from the public package data when `journey` is null. The PricingCard relies on `level` being non-null for code/milestones/summary display — always handle the no-journey case.
